...
- [TDSC' 22](#μdep-mutation-based-dependency-generation-for-precise-taint-analysis-on-android-native-code)：用变异算法测试native代码，找到输入输出相关性，从而给native代码建模。

之前一直做JNI相关的工作，跨语言的只有检查JNI程序的异常处理、检查本地代码原子性。把本地代码沙盒化也是相关的，但缺少了分析。
...
- [Sungho Lee](#sungho-lee)
- [CC' 05]()

除上述的学生外，其他相关的研究就是DOM建模，API使用等，分析主语言的API使用问题。和跨语言相关，但是只分析主语言，没有把客语言考虑。

### Ben Liblit（Amazon Web Services）
